      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.profitTrendData}>
                  ...
                  <YAxis tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} axisLine={{ stroke: 'hsl(var(--border))' }} tickFormatter={(value) => `R${(value / 1000).toFixed(0)}k`} />
                  <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', color: 'hsl(var(--foreground))' }} formatter={(value: number) => formatCurrency(value, 'ZAR')} />
                  <Area type="monotone" dataKey="revenue" stroke="hsl(239, 84%, 67%)" fillOpacity={1} fill="url(#colorRevenue)" name="Revenue" />
                  <Area type="monotone" dataKey="costs" stroke="hsl(187, 94%, 43%)" fillOpacity={1} fill="url(#colorCosts)" name="Costs" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={data.profitBreakdownData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value">
                    {data.profitBreakdownData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.color} />
                    ))}
                  </Pie>
                  <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', color: 'hsl(var(--foreground))' }} formatter={(value: number) => formatCurrency(value, 'ZAR')} />
                  <Legend verticalAlign="bottom" formatter={(value) => <span style={{ color: 'hsl(var(--foreground))' }}>{value}</span>} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.marginByShipment} barCategoryGap="60%">
                ...
                <YAxis tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 11 }} axisLine={{ stroke: 'hsl(var(--border))' }} tickFormatter={(value) => `${value}%`} />
                <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', color: 'hsl(var(--foreground))' }} formatter={(value: number) => [`${value.toFixed(1)}%`, 'Margin']} />
                <Bar dataKey="margin" fill="hsl(239, 84%, 67%)" radius={[4, 4, 0, 0]} maxBarSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-4 rounded-xl bg-muted/30 border border-border/30">
              ...
              <p className="text-2xl font-bold text-green-500">{formatCurrency(data.totalGrossProfit, 'ZAR')}</p>
            ...
            <div className="p-4 rounded-xl bg-muted/30 border border-border/30">
              ...
              <p className="text-2xl font-bold text-cyan-500">{formatCurrency(data.totalFxCommission, 'ZAR')}</p>
            ...
            <div className="p-4 rounded-xl bg-muted/30 border border-border/30">
              ...
              <p className="text-2xl font-bold text-purple-500">{formatCurrency(data.totalSpreadProfit, 'ZAR')}</p>
          ...
          <div className="mt-6 p-4 rounded-xl border border-destructive/30 bg-destructive/5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold text-destructive mt-1">-{formatCurrency(data.totalBankCharges, 'ZAR')}</p>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="glass-card p-6">
            ...
            <p className="text-3xl font-bold text-yellow-500">{formatCurrency(data.totalPendingPayments, 'ZAR')}</p>
            <p className="text-sm text-muted-foreground mt-2">Scheduled supplier payments awaiting completion</p>
          </div>
          <div className="glass-card p-6">
            <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
            ...
            <p className="text-3xl font-bold text-red-500">{formatCurrency(data.totalOwedToSuppliers, 'USD')}</p>
            <p className="text-sm text-muted-foreground mt-2">Total outstanding supplier balances</p>
          </div>
        </div>
              ...
              <tbody>
                {data.shipments.slice(0, 10).map((shipment) => (
                  <tr key={shipment.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                    ...
                    <td className={cn('py-3 px-4 text-right font-medium', (shipment.net_profit_zar || 0) >= 0 ? 'text-green-500' : 'text-red-500')}>
                      {formatCurrency(shipment.net_profit_zar || 0, 'ZAR')}
                    </td>
                    <td className="py-3 px-4 text-right text-muted-foreground">{(shipment.profit_margin || 0).toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function MetricCard({ title, value, icon: Icon, trend, highlight, description }: {
  title: string;
  value: string;
  icon: any;
  trend?: 'up' | 'down';
  ...
  return (
    <div className={cn('glass-card', highlight && 'ring-1 ring-primary/30')}>
      <div className="flex items-start justify-between mb-3">
        <div className="p-2 rounded-xl" style={{ background: 'linear-gradient(135deg, hsl(239 84% 67% / 0.2), hsl(187 94% 43% / 0.2))' }}>
          <Icon className="h-5 w-5 text-primary" />
        </div>
        {trend && (
          <span className={cn('trend-badge', trend === 'up' ? 'trend-up' : 'trend-down')}>
